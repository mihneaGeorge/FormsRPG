...
namespace Engine {
    public class Player : LivingCreature {
        ...
        public List<Armor> Armors {
            get {
                return Inventory.Where(
                    x => x.Details is Armor).Select(
                    x => x.Details as Armor).ToList();
            }
        }
        ...
        public string ToXmlString() {
            ...
            if (CurrentArmor != null) {
                XmlNode currentArmor = playerData.CreateElement("CurrentArmor");
                currentArmor.AppendChild(playerData.CreateTextNode(this.CurrentArmor.ID.ToString()));
                stats.AppendChild(currentArmor);
            }

            // "InventoryItems" child nodes
            XmlNode inventoryItems = playerData.CreateElement("InventoryItems");
            player.AppendChild(inventoryItems);

            //"InventoryItem" node for each item in player's inventory
            foreach (InventoryItem item in this.Inventory) {
                XmlNode inventoryItem = playerData.CreateElement("InventoryItem");

                XmlAttribute idAtrribute = playerData.CreateAttribute("ID");
                idAtrribute.Value = item.Details.ID.ToString();
                inventoryItem.Attributes.Append(idAtrribute);
            ...
            return playerData.InnerXml; //XML Document, as a string
        }
